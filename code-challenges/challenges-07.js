...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience > 1) {
            newData.push({ fullName, tech: arr[i].tech });
        }
    }
    return newData;
};
...
const applicationsStatics = (arr) => {
    ...
    arr.forEach(element => {
        ...
        if (element.yearsOfExperience <= 1 || (element.firstName == null && element.lastName == null)) {
            result.rejectedApplicants++;
        } else {
            switch (element.tech) {
                case "Python": result.python_devs++; break;
                case "Java": result.java_devs++; break;
                case "JS": result.javaScript_devs++; break;
                case ".Net": result.dotNet_devs++; break;
                default: break;
            }
        }
    });
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let k = 0; k < grade.classes.length; k++) {
            let classArr = grade.classes[k];
          let sum = 0
           for (let j = 0; j < classArr.classScores.length; j++) {
            sum += classArr.classScores[j]; 
           }
           let average = Math.floor(sum / classArr.classScores.length);
           classArr.avg = average;
        }
    }
    return data;
};
